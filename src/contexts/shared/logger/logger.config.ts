        ...
        const extra = Object.entries(rest)
          .reduce((prev: string[], [key, value]) => {
            if (value instanceof Error) {
              prev.push(`errname=${value.name}`, `error=${value.message}`);
              ...
              prev.push(`${key}=${String(value)}`);
            }
            return prev;
          }, [])
          .join("||");

        return `[${level.toUpperCase()}][${String(timestamp)}]||traceId=${traceId}||spanId=${spanId}||msg=${String(message)}${context ? `||context=${JSON.stringify(context)}` : ""}${extra ? `||${extra}` : ""}`;
      }),
    ),
